...
def run_with_threads(tur, log, main_turtle):
    ...
    thread_square = threading.Thread(target=draw_squares, args=(tur,))
    thread_circle = threading.Thread(target=draw_circles, args=(tur,))
    thread_triangle = threading.Thread(target=draw_triangles, args=(tur,))
    thread_rectangle = threading.Thread(target=draw_rectangles, args=(tur,))

    thread_square.start()
    thread_circle.start()
    thread_triangle.start()
    thread_rectangle.start()

    thread_square.join()
    thread_circle.join()
    thread_triangle.join()
    thread_rectangle.join()
